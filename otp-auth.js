...
(() => {
  ...
  window.renderLogin = function renderOtpLogin() {
    ...
    $('#verifyOtp').addEventListener('click', async () => {
      ...
      if (!/^\d{6}$/.test(token)) return notify('6-digit OTP দিন।');
      await run(async () => {
