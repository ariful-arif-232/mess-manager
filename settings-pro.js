...
(()=>{
  ...
  /* Both the admin and member views show this: every member can open the
     ...
     whole workspace. */
  const dataBackupCard=()=>`<section class="settings-card"><header><i>${icon('download')}</i><div><h3>Data backup</h3><p>Keep a readable copy before major changes</p></div></header>${row('Google Drive connection','One-time — lets Download Sheets create and update your mess\'s own Sheet.','<span class="gdrive-actions"><span id="googleDriveStatus" class="settings-verified" style="display:none"></span><button class="btn settings-inline-btn" id="connectGoogleDrive" style="display:none">Connect Google Drive</button></span>')}${row('Download Sheets','Opens your mess\'s own Google Sheet — Bazar and Khawa &amp; Taka, always up to date.','<button class="btn settings-inline-btn" id="exportMessData">Download Sheets</button>')}</section>`;
