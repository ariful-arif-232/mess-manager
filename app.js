...
async function persist(table,id,payload,entity){await run(async()=>{const query=id?client.from(table).update(payload).eq('id',id):client.from(table).insert(payload).select('id').single();const saved=assertResult(await query);const savedId=id||saved?.id;await logActivity(id?'update':'create',entity,savedId);if(!id&&table==='deposits'&&savedId)notifyDepositAdded(savedId);closeModal();await loadData();render()},`${entity.replace('_',' ')} saved.`)}
function notifyDepositAdded(depositId){client.functions.invoke('mess-activity-mail',{body:{action:'deposit-added',deposit_id:depositId}}).catch(err=>console.warn('Deposit email notify failed',err))}
