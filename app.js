...
const adminPages = new Set(['settings','activity']);
...
async function loadData(){
  ...
  const calls = [
    ...
    client.from('bazar_entries').select('*').gte('entry_date',start).lte('entry_date',end).order('entry_date',{ascending:false}),
  ...
  db.bazar = results[2].data.map(x => ({...x,date:x.entry_date}));
...
async function bootstrap(authSession){
  ...
  if(!session){ profile=null; mess=null; render(); return; }
  try {
...
function subscribeToMess(){
  const refresh = () => run(async()=>{await loadData(); render();});
  const tables = ['members','meals','bazar_entries','deposits','utility_bills','utility_bill_members','bazar_schedules','monthly_settlements'];
  realtimeChannel = client.channel(`mess:${profile.mess_id}`);
  tables.forEach(table=>{
    const options={event:'*',schema:'public',table};
    if(table!=='utility_bill_members') options.filter=`mess_id=eq.${profile.mess_id}`;
    realtimeChannel.on('postgres_changes',options,refresh);
  });
...
function nav(){
  ...
  const allowed = profile.role === 'admin' ? all : all.filter(([k]) => !['activity','settings'].includes(k));
...
function dashboard(c){const calc=calcMonth();const bazarTotal=db.bazar.reduce((s,x)=>s+Number(x.amount),0),dep=calc.reduce((s,x)=>s+x.deposit,0),util=db.utilities.reduce((s,x)=>s+Number(x.amount),0),due=calc.reduce((s,x)=>s+Math.max(0,-x.balance),0);c.innerHTML=`<div class="grid kpis"><div class="card kpi"><div class="label">মোট বাজার</div><div class="value">${money(bazarTotal)}</div></div><div class="card kpi"><div class="label">মোট জমা</div><div class="value">${money(dep)}</div></div><div class="card kpi"><div class="label">Utility Bills</div><div class="value">${money(util)}</div></div><div class="card kpi"><div class="label">মোট Due</div><div class="value">${money(due)}</div></div></div><div class="section-head"><h2>Member Summary</h2></div>${settlementTable(calc)}`;}
function members(c){const admin=profile.role==='admin';c.innerHTML=`<div class="section-head"><h2>সব Member</h2>${admin?'<button class="btn primary" data-add>+ Create Member Profile</button>':''}</div>${admin?'<div class="notice"><b>Member profile:</b> creates the database roster entry only. <b>Auth link:</b> separately enter an existing Supabase Auth user UUID to let that person sign in. Create Auth users in a trusted server or the Supabase Dashboard—never put a service_role key in this browser app.</div>':''}<div class="list">${db.members.map(m=>`<div class="list-item"><div><b>${esc(m.name)}</b><div class="muted">${m.active?'Active':'Inactive'} · ${esc(m.role)} · ${esc(m.email||'No email')}</div></div>${admin?`<div class="actions"><button class="btn" data-edit="${m.id}">Edit / Link Auth</button><button class="btn danger" data-toggle="${m.id}">${m.active?'Deactivate':'Activate'}</button></div>`:''}</div>`).join('')}</div>`;if(admin){c.querySelector('[data-add]').onclick=()=>memberModal();c.querySelectorAll('[data-edit]').forEach(x=>x.onclick=()=>memberModal(x.dataset.edit));c.querySelectorAll('[data-toggle]').forEach(x=>x.onclick=()=>toggleMember(x.dataset.toggle));}}
function memberModal(id){const m=db.members.find(x=>x.id===id)||{name:'',email:'',user_id:'',role:'member',active:true};modal(`<h2>${id?'Edit':'Add'} Member</h2><form id="memberForm"><div class="form-grid"><div class="field"><label>Name</label><input name="name" value="${esc(m.name)}" required maxlength="120"/></div><div class="field"><label>Email</label><input name="email" type="email" value="${esc(m.email||'')}"/></div><div class="field"><label>Auth user UUID (optional)</label><input name="user_id" value="${esc(m.user_id||'')}" placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"/></div><div class="field"><label>Role</label><select name="role"><option value="member">Member</option><option value="admin" ${m.role==='admin'?'selected':''}>Admin</option></select></div></div><div class="actions gap-top"><button class="btn primary">Save</button><button class="btn" type="button" data-close>Cancel</button></div></form>`);$('[data-close]').onclick=closeModal;$('#memberForm').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target),payload={mess_id:profile.mess_id,name:f.get('name').trim(),email:f.get('email').trim()||null,user_id:f.get('user_id').trim()||null,role:f.get('role')};await run(async()=>{const q=id?client.from('members').update(payload).eq('id',id):client.from('members').insert(payload);assertResult(await q);await logActivity(id?'update':'create','member',id,null);closeModal();await loadData();render();},'Member saved.');};}
...
function meals(c){const [start,end]=dateRange();const dates=[];for(let d=new Date(start+'T00:00:00Z'),last=new Date(end+'T00:00:00Z');d<=last;d.setUTCDate(d.getUTCDate()+1))dates.push(d.toISOString().slice(0,10));const visible=activeMembers();c.innerHTML=`<div class="card"><div class="muted">All members can view the shared meal sheet. Members may change only their own meal; admins may change any meal.</div></div><div class="section-head"><h2>Daily Meal</h2></div><div class="table-wrap"><table><thead><tr><th>Date</th>${visible.map(m=>`<th>${esc(m.name)}</th>`).join('')}</tr></thead><tbody>${dates.map(date=>`<tr><td>${date}</td>${visible.map(m=>{const x=db.meals.find(z=>z.date===date&&z.memberId===m.id),editable=profile.role==='admin'||m.id===profile.id;return `<td>${editable?`<button class="btn ${x?.on?'good':''}" data-meal-member="${m.id}" data-meal-date="${date}" data-current="${x?.on?'true':'false'}">${x?.on?'ON':'OFF'}</button>`:`<span class="pill">${x?.on?'ON':'OFF'}</span>`}</td>`;}).join('')}</tr>`).join('')}</tbody></table></div>`;c.querySelectorAll('[data-meal-member]').forEach(b=>b.onclick=()=>toggleMeal(b.dataset.mealMember,b.dataset.mealDate,b.dataset.current==='true'));}
...
function bazar(c){const admin=profile.role==='admin';c.innerHTML=`<div class="section-head"><h2>Bazar Entries</h2>${admin?'<button class="btn primary" data-add>+ Add Bazar</button>':''}</div>${entryTable(db.bazar,[['Date','date'],['Buyer','buyer'],['Items','items'],['Amount','amount']], 'bazar',admin)}`;if(admin)bindCrud(c,'bazar',bazarModal);}
function bazarModal(id){const x=db.bazar.find(z=>z.id===id)||{date:today(),buyer:'',items:'',amount:''};simpleModal(id,'Bazar',[['date','Date','date',x.date],['buyer','Buyer','text',x.buyer],['items','Items','textarea',x.items],['amount','Amount','number',x.amount]],async p=>{p.mess_id=profile.mess_id;p.entry_date=p.date;delete p.date;p.amount=Number(p.amount);await persist('bazar_entries',id,p,'bazar');});}
function deposits(c){const admin=profile.role==='admin';c.innerHTML=`<div class="section-head"><h2>Deposits</h2>${admin?'<button class="btn primary" data-add>+ Add Deposit</button>':''}</div>${entryTable(db.deposits,[['Date','date'],['Member',x=>memberName(x.memberId)],['Amount','amount'],['Note','note']],'deposits',admin)}`;if(admin)bindCrud(c,'deposits',depositModal);}
...
function utilities(c){const admin=profile.role==='admin';c.innerHTML=`<div class="section-head"><h2>Utility Bills</h2>${admin?'<button class="btn primary" data-add>+ Add Bill</button>':''}</div>${entryTable(db.utilities,[['Date','date'],['Type','type'],['Amount','amount'],['Shared By',x=>x.memberIds.map(memberName).join(', ')]],'utilities',admin)}`;if(admin)bindCrud(c,'utilities',utilityModal);}
...
function schedule(c){const admin=profile.role==='admin';c.innerHTML=`<div class="section-head"><h2>Bazar Schedule</h2>${admin?'<button class="btn primary" data-add>+ Add Schedule</button>':''}</div>${entryTable(db.schedules,[['Date','date'],['Assigned','names'],['Status',x=>x.done?'Done':'Pending']],'schedules',admin)}`;if(admin)bindCrud(c,'schedules',scheduleModal);}
...
function settlement(c){const calc=calcMonth();c.innerHTML=`<div class="card"><b>Calculation:</b> Bazar cost ÷ active meal units = meal rate. Utilities are divided among selected members. Deposit − total bill = advance/due.</div><div class="section-head"><h2>${state.month} Settlement</h2>${profile.role==='admin'?'<button class="btn primary" id="saveSettlement">Save draft</button>':''}</div>${settlementTable(calc)}`;if($('#saveSettlement'))$('#saveSettlement').onclick=()=>saveSettlements(calc);}
...
function entryTable(rows,columns,kind,editable=false){return `<div class="table-wrap"><table><thead><tr>${columns.map(([h])=>`<th>${h}</th>`).join('')}${editable?'<th>Actions</th>':''}</tr></thead><tbody>${rows.map(x=>`<tr>${columns.map(([,key])=>{let value=typeof key==='function'?key(x):x[key];if(key==='amount')value=money(value);return `<td>${esc(value||'-')}</td>`;}).join('')}${editable?`<td><div class="actions"><button class="btn" data-edit="${x.id}">Edit</button><button class="btn danger" data-delete="${x.id}" data-kind="${kind}">Delete</button></div></td>`:''}</tr>`).join('')||`<tr><td colspan="${columns.length+(editable?1:0)}" class="empty">No entries</td></tr>`}</tbody></table></div>`;}
