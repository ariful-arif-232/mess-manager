...
async function bootstrap(authSession){
  session = authSession;
  if(!session){ profile=null; mess=null; if(realtimeChannel) client.removeChannel(realtimeChannel); realtimeChannel=null; render(); return; }
  try {
...
function dashboard(c){let calc=calcMonth();const bazarTotal=db.bazar.reduce((s,x)=>s+Number(x.amount),0),dep=calc.reduce((s,x)=>s+x.deposit,0),util=db.utilities.reduce((s,x)=>s+Number(x.amount),0),due=calc.reduce((s,x)=>s+Math.max(0,-x.balance),0);c.innerHTML=`<div class="grid kpis"><div class="card kpi"><div class="label">মোট বাজার</div><div class="value">${money(bazarTotal)}</div></div><div class="card kpi"><div class="label">মোট জমা</div><div class="value">${money(dep)}</div></div><div class="card kpi"><div class="label">Utility Bills</div><div class="value">${money(util)}</div></div><div class="card kpi"><div class="label">মোট Due</div><div class="value">${money(due)}</div></div></div><div class="section-head"><h2>Member Summary</h2></div>${settlementTable(calc)}`;}
function members(c){const controls=profile.role==='admin';c.innerHTML=`<div class="section-head"><h2>সব Member</h2>${controls?'<button class="btn primary" data-add>+ Add Member</button>':''}</div>${controls?'<div class="notice">Create an Auth user securely in Supabase, then link its user UUID here. Never share temporary passwords.</div>':''}<div class="list">${db.members.map(m=>`<div class="list-item"><div><b>${esc(m.name)}</b><div class="muted">${m.active?'Active':'Inactive'} · ${esc(m.role)} · ${esc(m.email||'No email')}</div><div class="muted">${esc(m.phone||'No phone')} · Joined ${esc(m.join_date||'-')}</div></div>${controls?`<div class="actions"><button class="btn" data-edit="${m.id}">Edit</button><button class="btn danger" data-toggle="${m.id}">${m.active?'Deactivate':'Activate'}</button></div>`:''}</div>`).join('')}</div>`;if(!controls)return;c.querySelector('[data-add]').onclick=()=>memberModal();c.querySelectorAll('[data-edit]').forEach(x=>x.onclick=()=>memberModal(x.dataset.edit));c.querySelectorAll('[data-toggle]').forEach(x=>x.onclick=()=>toggleMember(x.dataset.toggle));}
function memberModal(id){const m=db.members.find(x=>x.id===id)||{name:'',email:'',phone:'',join_date:today(),user_id:'',role:'member',active:true};modal(`<h2>${id?'Edit':'Add'} Member</h2><form id="memberForm"><div class="form-grid"><div class="field"><label>Name</label><input name="name" value="${esc(m.name)}" required maxlength="120"/></div><div class="field"><label>Email</label><input name="email" type="email" value="${esc(m.email||'')}"/></div><div class="field"><label>Phone</label><input name="phone" type="tel" value="${esc(m.phone||'')}" maxlength="40"/></div><div class="field"><label>Join date</label><input name="join_date" type="date" value="${esc(m.join_date||today())}" required/></div><div class="field"><label>Auth user UUID (optional)</label><input name="user_id" value="${esc(m.user_id||'')}" placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"/></div><div class="field"><label>Role</label><select name="role"><option value="member">Member</option><option value="admin" ${m.role==='admin'?'selected':''}>Admin</option></select></div></div><div class="actions gap-top"><button class="btn primary">Save</button><button class="btn" type="button" data-close>Cancel</button></div></form>`);$('[data-close]').onclick=closeModal;$('#memberForm').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target),payload={mess_id:profile.mess_id,name:f.get('name').trim(),email:f.get('email').trim()||null,phone:f.get('phone').trim()||null,join_date:f.get('join_date'),user_id:f.get('user_id').trim()||null,role:f.get('role')};await run(async()=>{const q=id?client.from('members').update(payload).eq('id',id):client.from('members').insert(payload);assertResult(await q);await logActivity(id?'update':'create','member',id,null);closeModal();await loadData();render();},'Member saved.');};}
...
function meals(c){const [start,end]=dateRange();const dates=[];for(let d=new Date(start+'T00:00:00Z'),last=new Date(end+'T00:00:00Z');d<=last;d.setUTCDate(d.getUTCDate()+1))dates.push(d.toISOString().slice(0,10));const visible=activeMembers();c.innerHTML=`<div class="card"><div class="muted">প্রতিদিন meal ON/OFF করুন। Missing days default to OFF and are created securely when toggled.</div></div><div class="section-head"><h2>Daily Meal</h2></div><div class="table-wrap"><table><thead><tr><th>Date</th>${visible.map(m=>`<th>${esc(m.name)}</th>`).join('')}</tr></thead><tbody>${dates.map(date=>`<tr><td>${date}</td>${visible.map(m=>{const x=db.meals.find(z=>z.date===date&&z.memberId===m.id);return `<td><button class="btn ${x?.on?'good':''}" ${profile.role==='admin'?'':'disabled'} data-meal-member="${m.id}" data-meal-date="${date}" data-current="${x?.on?'true':'false'}">${x?.on?'ON':'OFF'}</button></td>`;}).join('')}</tr>`).join('')}</tbody></table></div>`;c.querySelectorAll('[data-meal-member]').forEach(b=>b.onclick=()=>toggleMeal(b.dataset.mealMember,b.dataset.mealDate,b.dataset.current==='true'));}
async function toggleMeal(memberId,date,current){if(!requireAdmin()) return;await run(async()=>{assertResult(await client.from('meals').upsert({mess_id:profile.mess_id,member_id:memberId,meal_date:date,enabled:!current,units:1},{onConflict:'member_id,meal_date'}));await logActivity('toggle','meal',null,{member_id:memberId,date,enabled:!current});await loadData();render();});}
function bazar(c){const controls=profile.role==='admin';c.innerHTML=`<div class="section-head"><h2>Bazar Entries</h2>${controls?'<button class="btn primary" data-add>+ Add Bazar</button>':''}</div>${entryTable(db.bazar,[['Date','date'],['Buyer',x=>memberName(x.buyer_member_id)||x.buyer],['Items',x=>x.items.map(i=>`${i.item_name} (${i.quantity} ${i.unit} × ${money(i.unit_price)})`).join(', ')],['Amount','amount']], 'bazar',controls)}`;if(controls)bindCrud(c,'bazar',bazarModal);}
function bazarItemRow(item={item_name:'',quantity:1,unit:'kg',unit_price:''}){return `<div class="bazar-item form-grid"><div class="field"><label>Item</label><input name="item_name" value="${esc(item.item_name)}" required maxlength="160"/></div><div class="field"><label>Quantity</label><input name="quantity" type="number" min="0.001" step="0.001" value="${esc(item.quantity)}" required/></div><div class="field"><label>Unit</label><input name="unit" value="${esc(item.unit)}" required maxlength="40"/></div><div class="field"><label>Unit price</label><input name="unit_price" type="number" min="0" step="0.01" value="${esc(item.unit_price)}" required/></div><button class="btn danger" type="button" data-remove-item>Remove</button></div>`;}
function bazarModal(id){const x=db.bazar.find(z=>z.id===id)||{date:today(),buyer_member_id:activeMembers()[0]?.id,items:[{}]};modal(`<h2>${id?'Edit':'Add'} Bazar</h2><form id="bazarForm"><div class="form-grid"><div class="field"><label>Date</label><input name="entry_date" type="date" value="${x.date}" required/></div><div class="field"><label>Buyer</label><select name="buyer_member_id">${activeMembers().map(m=>`<option value="${m.id}" ${x.buyer_member_id===m.id?'selected':''}>${esc(m.name)}</option>`).join('')}</select></div></div><div id="bazarItems">${(x.items.length?x.items:[{}]).map(bazarItemRow).join('')}</div><div class="actions gap-top"><button class="btn" type="button" id="addBazarItem">+ Item</button><b>Total: <span id="bazarTotal">৳0</span></b></div>${modalButtons()}</form>`);const items=$('#bazarItems');const updateTotal=()=>{$('#bazarTotal').textContent=money([...items.querySelectorAll('.bazar-item')].reduce((sum,row)=>sum+Number(row.querySelector('[name=quantity]').value||0)*Number(row.querySelector('[name=unit_price]').value||0),0));};const bindRows=()=>items.querySelectorAll('[data-remove-item]').forEach(button=>button.onclick=()=>{if(items.children.length>1)button.closest('.bazar-item').remove();updateTotal();});items.addEventListener('input',updateTotal);$('#addBazarItem').onclick=()=>{items.insertAdjacentHTML('beforeend',bazarItemRow());bindRows();updateTotal();};bindRows();updateTotal();$('[data-close]').onclick=closeModal;$('#bazarForm').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target),rows=[...items.querySelectorAll('.bazar-item')].map(row=>({item_name:row.querySelector('[name=item_name]').value.trim(),quantity:Number(row.querySelector('[name=quantity]').value),unit:row.querySelector('[name=unit]').value.trim(),unit_price:Number(row.querySelector('[name=unit_price]').value)}));await run(async()=>{const buyerId=f.get('buyer_member_id');const payload={mess_id:profile.mess_id,entry_date:f.get('entry_date'),buyer_member_id:buyerId,buyer:memberName(buyerId),items:rows.map(row=>row.item_name).join(', '),amount:0};let entryId=id;if(id){assertResult(await client.from('bazar_entries').update(payload).eq('id',id));assertResult(await client.from('bazar_items').delete().eq('bazar_entry_id',id));}else entryId=assertResult(await client.from('bazar_entries').insert(payload).select('id').single()).id;assertResult(await client.from('bazar_items').insert(rows.map(row=>({...row,bazar_entry_id:entryId}))));await logActivity(id?'update':'create','bazar',entryId);closeModal();await loadData();render();},'Bazar saved.');};}
function deposits(c){const controls=profile.role==='admin';c.innerHTML=`<div class="section-head"><h2>Deposits</h2>${controls?'<button class="btn primary" data-add>+ Add Deposit</button>':''}</div>${entryTable(db.deposits,[['Date','date'],['Member',x=>memberName(x.memberId)],['Amount','amount'],['Note','note']],'deposits',controls)}`;if(controls)bindCrud(c,'deposits',depositModal);}
...
function utilities(c){const controls=profile.role==='admin';c.innerHTML=`<div class="section-head"><h2>Utility Bills</h2>${controls?'<button class="btn primary" data-add>+ Add Bill</button>':''}</div>${entryTable(db.utilities,[['Date','date'],['Type','type'],['Amount','amount'],['Shared By',x=>x.memberIds.map(memberName).join(', ')]],'utilities',controls)}`;if(controls)bindCrud(c,'utilities',utilityModal);}
...
function settlement(c){let calc=calcMonth();c.innerHTML=`<div class="card"><b>Calculation:</b> Bazar cost ÷ active meal units = meal rate. Utilities are divided among selected members. Deposit − total bill = advance/due.</div><div class="section-head"><h2>${state.month} Settlement</h2>${profile.role==='admin'?'<button class="btn primary" id="saveSettlement">Save draft</button>':''}</div>${settlementTable(calc)}`;if($('#saveSettlement'))$('#saveSettlement').onclick=()=>saveSettlements(calc);}
...
function entryTable(rows,columns,kind,controls=profile.role==='admin'){return `<div class="table-wrap"><table><thead><tr>${columns.map(([h])=>`<th>${h}</th>`).join('')}${controls?'<th>Actions</th>':''}</tr></thead><tbody>${rows.map(x=>`<tr>${columns.map(([,key])=>{let value=typeof key==='function'?key(x):x[key];if(key==='amount')value=money(value);return `<td>${esc(value||'-')}</td>`;}).join('')}${controls?`<td><div class="actions"><button class="btn" data-edit="${x.id}">Edit</button><button class="btn danger" data-delete="${x.id}" data-kind="${kind}">Delete</button></div></td>`:''}</tr>`).join('')||`<tr><td colspan="${columns.length+(controls?1:0)}" class="empty">No entries</td></tr>`}</tbody></table></div>`;}
