...
const adminPages = new Set(['members','bazar','deposits','utilities','schedule','reports','settings','activity']);
...
function nav(){
  ...
  const allowed = profile.role === 'admin' ? all : all.filter(([k]) => ['dashboard','meals','settlement'].includes(k));
...
function members(c){c.innerHTML=`<div class="section-head"><h2>সব Member</h2><button class="btn primary" data-add>+ Add Member</button></div><div class="notice">Create an Auth user securely in Supabase, then link its user UUID here. Never share temporary passwords.</div><div class="list">${db.members.map(m=>`<div class="list-item"><div><b>${esc(m.name)}</b><div class="muted">${m.active?'Active':'Inactive'} · ${esc(m.role)} · ${esc(m.email||'No email')}</div></div><div class="actions"><button class="btn" data-edit="${m.id}">Edit</button><button class="btn danger" data-toggle="${m.id}">${m.active?'Deactivate':'Activate'}</button></div></div>`).join('')}</div>`;c.querySelector('[data-add]').onclick=()=>memberModal();c.querySelectorAll('[data-edit]').forEach(x=>x.onclick=()=>memberModal(x.dataset.edit));c.querySelectorAll('[data-toggle]').forEach(x=>x.onclick=()=>toggleMember(x.dataset.toggle));}
...
function bazar(c){c.innerHTML=`<div class="section-head"><h2>Bazar Entries</h2><button class="btn primary" data-add>+ Add Bazar</button></div>${entryTable(db.bazar,[['Date','date'],['Buyer','buyer'],['Items','items'],['Amount','amount']], 'bazar')}`;bindCrud(c,'bazar',bazarModal);}
