...
(()=>{
  ...
  const kpi=(label,value,kind)=>`<article class="kpi card mm-dashboard-kpi mm-dashboard-kpi-${kind}"><span class="kpi-icon mm-dashboard-kpi-icon mm-dashboard-icon-${kind}" aria-hidden="true"></span><div class="mm-dashboard-kpi-copy"><div class="label">${label}</div><div class="value">${money(value)}</div></div></article>`;
  ...
  window.dashboard=function compactDashboard(c){
    ...
    c.innerHTML=`<section class="kpis mm-dashboard-kpis" aria-label="Monthly mess summary">
      ...
      ${kpi('বাজার ফান্ড',bazarFund,'fund')}
      ${kpi('মোট Due',due,'due')}
