...
-- Before this migration each auth user could have at most one linked member.
...
begin
  ...
  -- Claim at most one matching unlinked roster row per workspace. This also
  -- protects against historical case-variant duplicate emails inside one mess.
  with claimable as (
    select distinct on (m.mess_id) m.id
    from public.members m
