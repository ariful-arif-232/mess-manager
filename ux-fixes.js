...
(() => {
  ...
  window.meals = function mealsFood(c){
    ...
    c.querySelector('#allEatToday')?.addEventListener('click',()=>setEveryoneEatingToday(visible));
  };
  ...
  function setEveryoneEatingToday(members){const date=today();return saveMeals(members.map(m=>{const existing=db.meals.find(x=>x.memberId===m.id&&x.date===date);return{mess_id:profile.mess_id,member_id:m.id,meal_date:date,units:Number(existing?.units||1),enabled:true};}),'আজ সবাই খাবে হিসেবে সেট হয়েছে।',{date,member_ids:members.map(m=>m.id),enabled:true,bulk:true});}
