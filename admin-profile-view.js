...
(()=>{
  ...
  const admins=()=>(db?.members||[])
    ...
    .slice()
    .sort((a,b)=>Number(a.active===false)-Number(b.active===false)||String(a.name||'').localeCompare(String(b.name||'')));
  ...
  function enhanceTriggers(){
    document.querySelectorAll('.settings-hero').forEach(hero=>{
      ...
      hero.setAttribute('aria-label','View mess administrator profiles');
      hero.dataset.adminProfileTrigger='1';
    });
  }
  ...
  function mount(content,label){
    ...
    document.body.insertAdjacentHTML('beforeend',`<div class="admin-profile-overlay" id="${OVERLAY_ID}" role="presentation"><section class="admin-profile-sheet" role="dialog" aria-modal="true" aria-label="${esc(label||'Admin profile')}">${content}</section></div>`);
  ...
  document.addEventListener('click',event=>{
    ...
    openAdminProfiles(trigger);
  },true);
  document.addEventListener('keydown',event=>{
    if(event.key==='Escape'&&document.getElementById(OVERLAY_ID)){closeViewer();return;}
    const trigger=event.target?.closest?.('.settings-hero');
    if(trigger&&(event.key==='Enter'||event.key===' ')){event.preventDefault();openAdminProfiles(trigger);}
  });

  const appRoot=document.getElementById('app')||document.documentElement;
  new MutationObserver(enhanceTriggers).observe(appRoot,{childList:true,subtree:true});
  enhanceTriggers();
