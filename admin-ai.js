...
(() => {
  ...
  function actionDetails(a){if(!a)return'';if(a.intent==='add_bazar')return `<div class="ai-review-list"><b>Bazar · ${escAi(a.date||'Today')}</b>${(a.items||[]).map(i=>`<div><span>${escAi(i.item_name)} · ${escAi(i.category)}</span><strong>${escAi(i.quantity)} ${escAi(i.unit)} × ৳${Number(i.unit_price||0).toFixed(2)}</strong></div>`).join('')}</div>`;if(a.intent==='add_deposit')return `<div class="ai-review-list"><div><span>Member</span><strong>${escAi(a.member_name||'আপনি')}</strong></div><div><span>Deposit</span><strong>৳${Number(a.amount||0).toFixed(2)}</strong></div></div>`;if(a.intent==='add_utility')return `<div class="ai-review-list"><div><span>${escAi(a.bill_type||'Utility')}</span><strong>৳${Number(a.amount||0).toFixed(2)}</strong></div><div><span>Members</span><strong>${escAi((a.member_names||[]).join(', '))}</strong></div></div>`;if(a.intent==='set_meal')return `<div class="ai-review-list"><div><span>${escAi(a.date||'Today')}</span><strong>${a.meal_enabled?'খাবে':'খাবে না'}</strong></div><div><span>Members</span><strong>${escAi((a.member_names||[]).join(', '))}</strong></div></div>`;return'';}
  ...
  async function callFunction({jsonBody,formBody}={}){if(!supabaseUrl||!anonKey)throw new Error('Supabase config পাওয়া যায়নি।');const {data:{session}}=await client.auth.getSession();if(!session?.access_token)throw new Error('Login session পাওয়া যায়নি। আবার sign in করুন।');const headers={Authorization:`Bearer ${session.access_token}`,apikey:anonKey};if(jsonBody)headers['Content-Type']='application/json';const r=await fetch(`${supabaseUrl}/functions/v1/admin-ai`,{method:'POST',headers,body:jsonBody?JSON.stringify(jsonBody):formBody});let data={};try{data=await r.json();}catch{}if(!r.ok)throw new Error(data?.error||data?.message||`AI service error (${r.status})`);if(data?.error)throw new Error(data.error);return data;}
